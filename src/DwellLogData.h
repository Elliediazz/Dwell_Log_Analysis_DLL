...
struct node {
  ...
  int dwell_index;
  int planned_dwell_s;
  int delivered_dwell_s;
  int dose_rate_Gy_per_s;
  string region_id;

  node* next; // points to the next node, or NULL if this is the last node.
  node* prev; // points to the previous node, or NULL if this is the head node.
};

// Doubly Linked List Invariant: The 'next' links for the tail node must be set to NULL
//The 'prev' links of a head node must be set to NULL 
//If the list is empty, both the 'prev' and 'next' node must be set to NULL 
//If a list conatins one single node, the 'prev' and 'next' node must point to the same node
